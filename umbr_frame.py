...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct 21 23:35:36 2016
...
def dist_open():
    while True:
        ...
        try:    
            ...
            print("Неверный формат файла!")
            fname.close()
def frame_prefilter(nparray):
    try:
        f_frame=int(input("Введите номер первого фрейма (По умолчанию: 0): "))
    except ValueError:
        f_frame=0
    try:    
        l_frame=1+int(input("Введите номер последнего фреймов (По умолчанию:"+str(len(nparray[:, 0])-1)+"): "))
    except:
...
if len(sys.argv)==1:
    ...
    nparray=dist_open_arg()
print(nparray)
try:   
    d=float(input("Введите величину шага между фреймами (нм)(По умолчанию: 0,1 нм):  "))
except ValueError:
    d=0.1
ff_frame=frame_filter(frame_prefilter(nparray), d)
print(ff_frame)
print(len(ff_frame))
olddir=str(os.getcwd())
u_n_dir=input("Введите имя подкаталога для хранения целевых фреймов: ")   
newdir=olddir+"/"+u_n_dir
print(newdir)
try: 
