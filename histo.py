...
для нормального отображения меток удалите символы форматирования
в исходном файле!""")       
        xvg_file=input(': ')        
        if xvg_file=='exit':
            joke()            
        ...
        try:    
            ...
            print("Неверный формат файла!")
            fname.close()
def name_label(xvg_file):
    str_label=str(open(xvg_file).readlines()[13])
    open(xvg_file).close()
    i=str_label.index('"')
    j=str_label.rindex('"')
    ...
    return label
def name_x(xvg_file):
    str_label=str(open(xvg_file).readlines()[14])
    open(xvg_file).close()
    i=str_label.index('"')
    j=str_label.rindex('"')
    ...
    return name_x
def name_y(xvg_file):
    str_label=str(open(xvg_file).readlines()[15])
    open(xvg_file).close()
    i=str_label.index('"')
    j=str_label.rindex('"')
...
def print_graph(x, nparray):
    if  nparray.shape[1]==2:
        ...
        for i in range(1, nparray.shape[1]):
            y = nparray[:,i]
            ax.plot(x, y, label=name_y(xvg_file)+str(i))
def save_graph():
    while True:
        file_name_g=str(input('Введите имя файла для сохранения графика (*.eps, *.jpeg, *.jpg, *.pdf, *.pgf, *.png, *.ps, *.raw, *.rgba, *.svg, *.svgz, *.tif, *.tiff)(По умолчанию: .png): '))
        try:
...
if len(sys.argv)==3:
    ...
    print('Показывать ли техническую легенду? Введите "yes" для показа: ')
    try:        
        if str(input())=='yes':
