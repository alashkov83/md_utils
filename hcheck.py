    ...
    for ih, ah in enumerate(atom):
        if element[ih] == ' H':
            ...
            if r > max_dist and check_link(occupancy[ih], oc, alter[ih], alt):
                print(
                    """Похоже, что водород {1:s}:{0:s} остатка {2:s}{8:s}{3:d} цепи {4:s} не связан с тяжёлыми атомами! 
Минимальное расстояние c {5:s}{9:s}{6:s} = {7:6.2f} \u212b!""".format(
                        atom[ih], alter[ih], res_name[ih], resn_curent, chain_id_curent, alt, at, r,
                        '' if alter[ih] == ' ' else ':', '' if alt == ' ' else ':'))
            elif r < min_dist and check_link(occupancy[ih], oc, alter[ih], alt):
                print(
                    """Похоже, что водород {1:s}:{0:s} остатка {2:s}{8:s}{3:d} цепи {4:s} конфликтует с {5:s}{9:s}{6:s}!
